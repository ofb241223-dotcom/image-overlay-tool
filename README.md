...
- 支持 PNG、JPG、WebP、BMP、SVG 输入。
- 支持拖放导入、最近文件、撤销/重做、重置和导出提示。
- 支持导出 PNG、JPG、WebP、BMP、SVG。
- 导出检查会提示当前格式是否有损、是否保留透明，以及 SVG 是否只是嵌入图片。
- 默认中文界面，可切换英文。
...
当前 SVG 导出是把最终合成后的 PNG 嵌入 SVG 文件，不是把 JPG/PNG 矢量化。PNG 和无损 WebP 会保留透明；JPG 会有损压缩并铺白底。
