...
This repository is a compact Python image-overlay utility. The main application starts from `merge_logo_gui.py`; it delegates argument parsing and export logic to `image_overlay_tool/cli.py`, keeps Pillow image processing in `image_overlay_tool/core.py`, and uses `image_overlay_tool/gui_qt.py` for the PySide6/Qt editor UI. User-supplied images are selected at runtime or passed with CLI flags. Treat `__pycache__/` as generated output and leave it out of reviews and commits.

## Build, Test, and Development Commands

- `python3 merge_logo_gui.py` launches the PySide6/Qt GUI for selecting a base image and overlay image.
...
- `python3 -m pip install -r requirements.txt` installs Pillow, PySide6, and packaging tools in a fresh environment.
...
Use Python 3 style with 4-space indentation, type hints, and small helper functions. Keep dataclasses such as `Placement` immutable unless a real stateful need appears. Use `snake_case` for functions, variables, and CLI flags; use `PascalCase` for PySide6/Qt classes. Keep bilingual UI strings in `image_overlay_tool/i18n.py`, and add matching `zh` and `en` entries for every new interface label or message.
