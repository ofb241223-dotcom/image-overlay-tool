...
class PreviewCanvas(QWidget):
    ...
    def _rotate_handle_rect(self) -> QRectF | None:
        overlay_rect = self._overlay_rect(self.active_layer_idx)
        if overlay_rect is None:
            return None
        handle_size = 16.0
        center_x = overlay_rect.center().x()
        center_y = overlay_rect.top() - 24.0
        return QRectF(
    ...
    @staticmethod
    def _angle_between(center: QPointF, point: QPointF) -> float:
    ...
    def paintEvent(self, _event: object) -> None:
        ...
        for i, item in enumerate(self.overlays):
            ...
            if i == self.active_layer_idx:
                ...
                rotate_handle = self._rotate_handle_rect()
    ...
    def wheelEvent(self, event) -> None:
        ...
        next_placement = replace(current, width=next_width)
        self.placement_changed.emit((self.active_layer_idx, next_placement))
        self.placement_committed.emit((self.active_layer_idx, next_placement))
        event.accept()

    def mouseDoubleClickEvent(self, event: QMouseEvent) -> None:
        if event.button() == Qt.MouseButton.LeftButton and self.base_image is not None:
            hit_index = self._hit_overlay_index(event.position())
    ...
    def _select_layer_at_point(self, point: QPointF) -> int | None:
    ...
    def mousePressEvent(self, event: QMouseEvent) -> None:
        ...
        if event.button() != Qt.MouseButton.LeftButton:
            return
        hit_index = self._select_layer_at_point(point)
        if hit_index is None:
            return
        ...
        if rotate_handle is not None and rotate_handle.contains(point):
            self._drag_mode = "rotate"
            self._drag_start_center = overlay_rect.center() if overlay_rect is not None else None
            if self._drag_start_center is not None:
                self._drag_start_angle = self._angle_between(self._drag_start_center, point)
        elif handle_rect is not None and handle_rect.contains(point):
        ...
        self._drag_layer_idx = hit_index
        self._drag_start_pos = (point.x(), point.y())
    ...
    def mouseMoveEvent(self, event: QMouseEvent) -> None:
        ...
        if not (0 <= self._drag_layer_idx < len(self.overlays)):
            return
        ...
        if self._drag_mode == "move":
            ...
            ov_w_t, ov_h_t = get_transformed_overlay_size(start.width, self.overlays[self._drag_layer_idx].image.size, start.rotation)
            ...
            if self._drag_start_center is None:
                return
            current_angle = self._angle_between(self._drag_start_center, event.position())
            rotation = start.rotation + current_angle - self._drag_start_angle
            overlay_size = self.overlays[self._drag_layer_idx].image.size
            old_w, old_h = get_transformed_overlay_size(start.width, overlay_size, start.rotation)
            new_w, new_h = get_transformed_overlay_size(start.width, overlay_size, rotation)
            next_placement = replace(
                start,
                x=round(start.x + old_w / 2 - new_w / 2),
                y=round(start.y + old_h / 2 - new_h / 2),
                rotation=rotation,
            )

        self.placement_changed.emit((self._drag_layer_idx, next_placement))
        event.accept()
    ...
    def mouseReleaseEvent(self, event: QMouseEvent) -> None:
        ...
        released_layer_idx = self._drag_layer_idx
        ...
        self._drag_start_pan = None
        if 0 <= released_layer_idx < len(self.overlays):
            self.placement_committed.emit((released_layer_idx, self.overlays[released_layer_idx].placement))
        self._update_cursor(event)
        event.accept()
...
class OverlayMainWindow(QMainWindow):
    ...
    def _build_ui(self) -> None:
        ...
        self.preview.placement_changed.connect(lambda event: self._set_placement(event[0], event[1], record=False))
    ...
    def _build_side_panel(self) -> QWidget:
        ...
        self.x_slider = QSlider(Qt.Orientation.Horizontal, panel)
        self.y_slider = QSlider(Qt.Orientation.Horizontal, panel)
        self.width_slider = QSlider(Qt.Orientation.Horizontal, panel)
        for slider in (self.x_slider, self.y_slider, self.width_slider):
            slider.setRange(0, 1)
        self.x_slider.valueChanged.connect(lambda value: self.x_spin.setValue(value))
        self.y_slider.valueChanged.connect(lambda value: self.y_spin.setValue(value))
        self.width_slider.valueChanged.connect(lambda value: self.width_spin.setValue(max(1, value)))

        def paired_control(slider: QSlider, spin: QSpinBox | QDoubleSpinBox) -> QWidget:
            box = QWidget(panel)
            row = QHBoxLayout(box)
        ...
        self.opacity_spin = QSpinBox(panel)
        self.opacity_spin.setRange(0, 100)
        self.opacity_spin.setSuffix("%")
        self.opacity_spin.setValue(100)
        self.opacity_slider.valueChanged.connect(lambda value: self.opacity_spin.setValue(value))
        self.opacity_spin.valueChanged.connect(self._on_numeric_control_changed)
        opacity_box = paired_control(self.opacity_slider, self.opacity_spin)
        self.remove_white_check = QCheckBox(panel)
        self.remove_white_check.stateChanged.connect(self._on_numeric_control_changed)
        ...
        self.rotation_slider = QSlider(Qt.Orientation.Horizontal, panel)
        self.rotation_slider.setRange(-1800, 1800)
        self.rotation_slider.valueChanged.connect(lambda value: self.rotation_spin.setValue(value / 10.0))
        rotation_box = paired_control(self.rotation_slider, self.rotation_spin)
        ...
        placement_layout.addRow(self.x_label, paired_control(self.x_slider, self.x_spin))
        placement_layout.addRow(self.y_label, paired_control(self.y_slider, self.y_spin))
        placement_layout.addRow(self.width_label, paired_control(self.width_slider, self.width_spin))
    ...
    def _update_live_placement(self) -> None:
        ...
        if 0 <= self.active_layer_idx < len(self.overlays):
            ...
            self.x_slider.setValue(placement.x)
            self.y_slider.setValue(placement.y)
            self.width_slider.setValue(placement.width)
            self.opacity_slider.setValue(round(placement.opacity * 100))
            self.opacity_spin.setValue(round(placement.opacity * 100))
            self.rotation_spin.setValue(placement.rotation)
            self.rotation_slider.setValue(round(placement.rotation * 10))
    ...
    def _sync_controls_from_state(self) -> None:
        ...
        if has_active:
            ...
            overlay_w, overlay_h = get_transformed_overlay_size(
                placement.width,
                self.overlays[self.active_layer_idx].image.size,
                placement.rotation,
            )
            max_x = max(1, self.base_image.width - overlay_w) if self.base_image else 1
            max_y = max(1, self.base_image.height - overlay_h) if self.base_image else 1
            max_width = max(
                2,
                get_max_overlay_width(
                    ...
                    self.overlays[self.active_layer_idx].image.size,
                    placement.rotation,
                ) if self.base_image else placement.width,
            )
            ...
            self.width_spin.setRange(1, 2)
            self.x_slider.setRange(0, 1)
            self.y_slider.setRange(0, 1)
            self.width_slider.setRange(1, 2)
